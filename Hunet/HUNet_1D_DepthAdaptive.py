"""HUNet-1D DepthAdaptive: 1D ultrasound CS reconstruction with adaptive shrinkage.

This file is a standalone upgraded copy of HUNet_1D.py.
Key improvements:
  - Depth-adaptive bottleneck threshold map (feature-gated + depth curve)
  - Same unfolding/U-Net/DFFM backbone and interface as the original HUNet
  - Loss-friendly hook `recommended_loss_cfg()` for depth-weighted training
"""
...
class AdaptiveThresholdMap1D(nn.Module):
    """Generate per-position threshold scaling from bottleneck features.

    thr = base_gamma * local_scale(feat) * depth_scale(pos)
    """

    def __init__(self, channels, hidden_ratio=0.5, scale_min=0.5,
                 scale_max=1.5, init_depth_slope=-0.3):
        super().__init__()
        ...
        self.depth_slope = nn.Parameter(torch.tensor(init_depth_slope))

    def forward(self, feat, base_thr):
        # feat/base_thr: (B, L, C)
        gate = torch.sigmoid(self.mlp(self.norm(feat)))
        local_scale = self.scale_min + (self.scale_max - self.scale_min) * gate

        L = feat.shape[1]
        depth = torch.linspace(
            0.0, 1.0, L, device=feat.device, dtype=feat.dtype
        ).view(1, L, 1)
        depth_scale = torch.exp(self.depth_slope * depth)
        depth_scale = depth_scale / depth_scale.mean(dim=1, keepdim=True).clamp(min=1e-6)

        thr = base_thr * local_scale * depth_scale
        return thr.clamp_min(1e-6)
...
class HomotopyStage1DDepthAdaptive(nn.Module):
    """Single homotopy unfolding stage with adaptive thresholding."""

    def __init__(self, stage_no, embed_dim=32, depth=4, nhead=4,
                 window_size=32, mlp_ratio=2.0, swin_depth=2,
                 adaptive_thr=True, thr_hidden_ratio=0.5,
                 thr_scale_min=0.5, thr_scale_max=1.5,
                 thr_init_depth_slope=-0.3,
                 gamma_init=0.03, gamma_decay=0.85, gamma_floor=1e-4):
        super().__init__()
        self.depth = depth
        ...
        gamma0 = max(gamma_init * (gamma_decay ** stage_no), gamma_floor * 2.0)
        gamma0_t = torch.full((1, 1, bottleneck_dim), gamma0)
        self.gamma_raw = nn.Parameter(_inverse_softplus(gamma0_t - gamma_floor))

        self.thr_map = (
            AdaptiveThresholdMap1D(
                bottleneck_dim,
                hidden_ratio=thr_hidden_ratio,
                scale_min=thr_scale_min,
                scale_max=thr_scale_max,
                init_depth_slope=thr_init_depth_slope,
            )
            if adaptive_thr else None
        )
        self.last_thr_mean = (F.softplus(self.gamma_raw) + self.gamma_floor).mean().detach()
    ...
    def forward(self, x, y, op):
        ...
        for i, (x_e, dec_blocks) in enumerate(zip(x_ms, self.decoder_blocks)):
            if i == 0:
                B_sz, Lb, Cb = x_e.shape
                base_gamma = F.softplus(self.gamma_raw) + self.gamma_floor
                base_thr = base_gamma.expand(B_sz, Lb, Cb)
                thr = self.thr_map(x_e, base_thr) if self.thr_map is not None else base_thr
                self.last_thr_mean = thr.mean().detach()
                x_e = soft_threshold(x_e, thr)
...
class HUNet1DDepthAdaptive(nn.Module):
    """Depth-aware HUNet-1D with adaptive bottleneck thresholding.

    Improvements over the baseline HUNet-1D:
      - Adaptive threshold map: per-position/per-channel shrinkage in bottleneck
      - Learnable depth curve: encourages depth-aware threshold adjustment
      - Fully compatible with CombinedLoss; recommended to use depth_weight
    """
    ...
    def __init__(self, num_stages=7, depth=4, embed_dim=32,
        ...
        self.stages = nn.ModuleList([
            HomotopyStage1DDepthAdaptive(
                stage_no=i, embed_dim=embed_dim, depth=depth,
                nhead=nhead, window_size=window_size,
                mlp_ratio=mlp_ratio, swin_depth=swin_depth,
                adaptive_thr=adaptive_thr, thr_hidden_ratio=thr_hidden_ratio,
                thr_scale_min=thr_scale_min, thr_scale_max=thr_scale_max,
                thr_init_depth_slope=thr_init_depth_slope,
                gamma_init=gamma_init, gamma_decay=gamma_decay,
    ...
    @staticmethod
    def recommended_loss_cfg():
        """Recommended training loss for this model."""
        return {
            "loss_mode": "nmse",
            "depth_weight": "linear",
            "depth_weight_alpha": 2.0,
        }
...
# Keep a short alias for external imports.
HUNet1D_DA = HUNet1DDepthAdaptive
