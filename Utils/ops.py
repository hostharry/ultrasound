...
def smooth_soft_threshold(x, thr, tau: float = 0.01):
    """逐元素平滑软阈值 (Softplus 近似).

    sign(x) · tau · softplus((|x| - thr) / tau)

    - tau -> 0   : 趋近原 soft_threshold (硬阈值, dead zone 输出严格为 0)
    - tau > 0    : 处处可微, dead zone 也有非零梯度, 解决 HASA 的 ∂L/∂thr=0 死锁

    极限行为:
        |x| >> thr  : 输出 ≈ sign(x)·(|x|-thr)        (与原版一致)
        |x| << thr  : 输出 ≈ sign(x)·tau·exp((|x|-thr)/tau)  (微小泄漏)

    建议: tau 取在 thr 同量级或更小, 一般 0.001 ~ 0.02.
    与 soft_threshold 接口完全兼容 (thr 可以是标量或与 x 同形状的张量).
    """
    abs_x = torch.abs(x)
    return torch.sign(x) * tau * F.softplus((abs_x - thr) / tau)
