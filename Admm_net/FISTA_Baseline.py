"""1D 对比模型: HASA-FISTA (从 FISTA_Baseline_2D.py 改编).

设计要点:
  - Data consistency: z = v - rho * A^T(A(v)-y)
  - HASA 权重: 逐样本 lambda_tv / lambda_wav / alpha
...
class HASAWeightFISTA1D(nn.Module):
    """HASA-FISTA 专用权重网络 (1D): 输出 lambda_tv/lambda_wav/alpha."""

    def __init__(self, hidden_ch=16, num_layers=2, inner_ks=5):
        super().__init__()
        layers = [nn.Conv1d(1, hidden_ch, 3, padding=1), nn.ReLU(inplace=True)]
        inner_pad = inner_ks // 2
        for _ in range(num_layers - 1):
            layers.extend([
                nn.Conv1d(hidden_ch, hidden_ch, inner_ks, padding=inner_pad),
                nn.ReLU(inplace=True),
            ])
        self.feat_net = nn.Sequential(*layers)
        self.head_tv = nn.Sequential(nn.Conv1d(hidden_ch, 1, 1), nn.Softplus())
        self.head_wav = nn.Sequential(nn.Conv1d(hidden_ch, 1, 1), nn.Softplus())
        self.head_alpha = nn.Sequential(nn.Conv1d(hidden_ch, 1, 1), nn.Sigmoid())
...
class ISTAProx1D_Dual(nn.Module):
    """1D 双分支 ISTA Prox.
    ...
    def __init__(self, feat_ch=32, k=3):
        ...
        self.E1_tv = nn.Conv1d(1, feat_ch, k, padding=pad)
        self.E2_tv = nn.Conv1d(feat_ch, feat_ch, k, padding=pad)
        self.D1_tv = nn.Conv1d(feat_ch, feat_ch, k, padding=pad)
        self.D2_tv = nn.Conv1d(feat_ch, 1, k, padding=pad)

        self.E1_wav = nn.Conv1d(1, feat_ch, k, padding=pad)
        self.E2_wav = nn.Conv1d(feat_ch, feat_ch, k, padding=pad)
        self.D1_wav = nn.Conv1d(feat_ch, feat_ch, k, padding=pad)
        self.D2_wav = nn.Conv1d(feat_ch, 1, k, padding=pad)

    @staticmethod
    def _expand_thr(thr, feat):
        return thr.expand(-1, feat.shape[1], -1)
...
class HASA_FISTA_Block_1D(nn.Module):
    """单层 HASA-FISTA block (1D)."""
    ...
    def __init__(self, weight_ctor, feat_ch=32, prox_k=3):
        ...
        self.prox = ISTAProx1D_Dual(feat_ch=feat_ch, k=prox_k)
...
class HASA_FISTA_Net_1D(nn.Module):
    """1D HASA-FISTA 网络."""
    ...
    def __init__(self, layer_num=12, hasa_ctor=None, feat_ch=32, prox_k=3):
        ...
        self.blocks = nn.ModuleList(
            [HASA_FISTA_Block_1D(hasa_ctor, feat_ch=feat_ch, prox_k=prox_k) for _ in range(layer_num)]
        )
